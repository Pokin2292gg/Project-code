<template>
    <Navbar />
    <section class="about">
        <h1 class="heading"> Restau<span>rant </span> </h1>
        <a-table :columns="columns" :data-source="data" :rowKey="(record) => record.id">
            ...
            </template>            <template #rating="{ record }">
                <star-rating :rating="record?.rating" />

            </template>
        </a-table>


        <a-button type="primary" @click="showModal">เพิ่มร้านอาหาร</a-button>
    </section>

    <a-modal v-model:visible="modalVisible" title="เพิ่ม/แก้ไข ร้านอาหาร" @ok="handleOk" okText="บันทึก"
        cancelText="ยกเลิก">
        <a-row>
            <a-col :span="6"><label>ร้านอาหาร</label></a-col>
            <a-col :span="18">
                <a-input v-model:value="book.name" placeholder="ร้านอาหาร" />
            </a-col>
        </a-row>
        <a-row>
            <a-col :span="6"><label>คะแนนรีวิว</label></a-col>
            <a-col :span="18">
                <star-rating v-model:rating="book.rating" />
...
export default {
    ...
    methods: {
        ...
        showConfirmDelete(id) {
            ...
            Modal.confirm({
                title: 'ลบข้อมูล',
                content: 'คุณต้องการลบ ' + book.name + ' หรือไม่?',
                okText: 'ลบ',
                cancelText: 'ยกเลิก',
                onOk: async () => {
                    try {
